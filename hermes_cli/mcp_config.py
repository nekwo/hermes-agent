...
from tools.mcp_tool import _ENV_VAR_NAME_RE, _ENV_VAR_PATTERN, _env_ref_name
...
def _resolve_mcp_server_config(config: dict) -> dict:
    ...
    # Credential isolation first: only fall back to the machine-wide
    # ``~/.hermes/.env`` when no scoped secret source is active. Loading the
    # dotenv unconditionally would leak the machine's keys into a scoped probe.
    ...
    # ``${roots.<name>}`` / ``${exe_suffix}`` expansion is a *path* concern, not
    # a secret one, so it runs regardless of the secret scope and before the
    # ``${ENV}`` interpolation below.
    resolved = config
    try:
