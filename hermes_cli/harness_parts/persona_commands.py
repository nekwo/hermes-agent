    ...
    if session_id and is_canonical_session_persistence(session_db):
        ...
        if (
            ...
            if getattr(args, "stream", False):
                ...
                print(emit_json(data) if args.json else data["error"])
            return 2
        # TASK-BOUND ROOT GUARD. The explicit root has just been resolved to the
        # instance that OWNS it, which is the only place the runtime knows which
        # lifecycle the target is in — so it is the only honest place to refuse a
        # bare operator chat aimed at the goal graph's own instances. See
        # `_task_bound_chat_root_refusal` for why that route is gone and which
        # lanes keep it.
        task_bound_refusal = _task_bound_chat_root_refusal(
            instance_store,
            owner_instance=owner_instance,
            persona_id=normalized_persona,
            session_id=session_id,
            mission_context=_mission_chat_carries_mission_context(
                args, relay_chain=relay_chain_in
            ),
        )
        if task_bound_refusal is not None:
            if getattr(args, "stream", False):
                _emit_chat_final(task_bound_refusal)
            else:
                print(
                    emit_json(task_bound_refusal)
                    if args.json
                    else task_bound_refusal["error"]
                )
            return 2
...
#: The instance lifecycle that belongs to the goal graph / daemon rather than to
#: an operator conversation. One spelling, read by the guard below; the store
#: writes it in ``ensure_for_goal`` / ``_bind`` and the mission-chat handler
#: stamps it whenever a send carries ``--task``/``--goal``.
TASK_BOUND_INSTANCE_MODE = "task_bound"


def _mission_chat_carries_mission_context(args, *, relay_chain) -> bool:
    """Is this send on a MISSION lane rather than the bare operator-chat one?

    The single distinction behind :func:`_task_bound_chat_root_refusal`, and the
    only thing that separates "the daemon/graph/relay is talking to its own
    task-bound instance" from "an operator typed at the wrong target". Both
    signals are ENVELOPE facts the caller had to supply deliberately, never
    ambient state:

    * a non-empty ``relay_chain`` — the explicit agent-relay envelope
      (``agent_chat_send`` always carries it, seeded from the running turn's
      chain; a root-node engine turn is the outermost hop and its onward sends
      carry the chain naming it). A bare operator CLI/serve send has NONE, so
      the chain is empty exactly when nobody but the operator is speaking.
    * an explicit ``--task``/``--goal`` — mission context named on the turn
      itself. This is not a proxy: the handler already treats those arguments as
      the authority that MAKES an instance task-bound (it stamps
      ``mode="task_bound"`` from them further down), so a send carrying one is by
      definition addressing the mission lane.

    Deliberately reads the INCOMING chain, not the turn's outgoing one: the
    handler appends the target persona before seeding the ContextVars, so the
    outgoing chain is never empty and would exempt every send ever made.
    """

    if tuple(relay_chain or ()):
        return True
    return bool(
        safe_assignment_token(getattr(args, "task_id", None))
        or safe_assignment_token(getattr(args, "goal_id", None))
    )


def _chat_mode_redirect_targets(
    instance_store,
    *,
    persona_id: str,
    exclude_instance_id: str | None,
) -> list[dict[str, object]]:
    """The persona's chat-mode on-level instances, as redirect candidates.

    A refusal that only says "not here" costs the caller a roster round-trip to
    find out where "here" is, so the guard names the alternatives it already has
    in hand. Read-only and best-effort: a store that cannot answer yields an
    empty list, and the refusal still stands on its own — the redirect is help,
    never the reason.

    "Chat-mode" is spelled as the COMPLEMENT of the lifecycle being refused
    (anything but ``task_bound``), deliberately rather than as an allow-list of
    conversational modes. A narrower rule would silently drop a legitimate
    target that happens to sit in ``configured`` — a placement between binds —
    and a redirect that omits the one instance the operator should have used is
    worse than no redirect at all. Everything this list offers is, by
    construction, a target this very guard would let through.

    ``default_chat_session_id`` is carried when the instance HAS one (it is a
    field on the row already read, so this costs nothing) and reported as
    ``None`` otherwise — an instance that has never chatted has no root to name,
    and fabricating one would hand the caller an id the ``unknown_chat_session``
    guard would immediately refuse. Rows that already carry a root sort FIRST,
    because those are the ones the caller can act on without minting anything.
    """

    try:
        rows = list(instance_store.list_all())
    except Exception:  # pragma: no cover - defensive; the refusal does not need this
        return []
    targets = [
        {
            "persona_instance_id": instance.id,
            ...
            or instance.id,
            "default_chat_session_id": safe_assignment_text(
                getattr(instance, "default_chat_session_id", None), limit=240
            )
            or None,
        }
        for instance in rows
        if getattr(instance, "persona_id", None) == persona_id
        and instance.id != exclude_instance_id
        and str(getattr(instance, "mode", "") or "").lower()
        != TASK_BOUND_INSTANCE_MODE
    ]
    targets.sort(
        key=lambda target: (
            target["default_chat_session_id"] is None,
            str(target["persona_instance_id"]),
        )
    )
    return targets


def _task_bound_chat_root_refusal(
    instance_store,
    *,
    owner_instance,
    persona_id: str,
    ...
    """Refuse a bare operator chat aimed at a TASK-BOUND instance's chat root.

    THE ROUTE IS REMOVED, not deprecated (operator ruling, 2026-07-29). The bare
    task-bound rows — ``personainst_qa``, ``personainst_dev``,
    ``personainst_neko_supervisor``, ``personainst_goal_*`` — belong to the goal
    graph and its daemon. Messaging their chat roots worked MECHANICALLY, which
    is precisely what made it worth removing: the turn landed, the reply came
    back, and the Mission Control console then misattributed the thread, so the
    operator's own chat rode a broken mission path with no error to read. The
    canonical operator target is the chat-mode on-level instance
    (``personainst_<role>_agent_<hash>``), and this names it.

    ``mission_context`` is the whole of the exemption, and it is deliberately
    decided by the CALLER's envelope rather than re-derived here (see
    :func:`_mission_chat_carries_mission_context`): the daemon/graph/worker
    lanes, root-node engine turns, and every agent-relay hop legitimately speak
    to these instances, and none of them is what the operator asked to remove.
    Only the bare operator route — no relay chain, no ``--task``/``--goal`` — is
    gone.

    Returns ``None`` for every send that may proceed, so the call site reads as
    one refusal check rather than an inverted condition.
    """

    if mission_context:
        return None
    mode = str(getattr(owner_instance, "mode", "") or "").lower()
    if mode != TASK_BOUND_INSTANCE_MODE:
        return None
    owner_id = safe_assignment_token(getattr(owner_instance, "id", None)) or None
    redirect_targets = _chat_mode_redirect_targets(
        instance_store, persona_id=persona_id, exclude_instance_id=owner_id
    )
    if redirect_targets:
        named = "; ".join(
            (
                f"{target['persona_instance_id']} "
                f"(root {target['default_chat_session_id']})"
                if target["default_chat_session_id"]
                else str(target["persona_instance_id"])
            )
            for target in redirect_targets[:5]
        )
        next_expected = (
            "re-send to this persona's chat-mode on-level instance instead: "
            f"{named}"
        )
    else:
        next_expected = (
            "this persona has no chat-mode on-level instance yet; mint one with "
            "`persona instance open-chat --new-session --idempotency-key <key>` "
            "and send to the root it returns"
        )
    return {
        "ok": False,
        "capability_id": "mission.chat.message",
        "execution_state": "rejected",
        "error_kind": "task_bound_chat_root",
        "error": (
            "task-bound instance chat roots are not an operator chat route: "
            f"{session_id}"
        ),
        "session_id": session_id,
        "persona_id": persona_id,
        "persona_instance_id": owner_id,
        "persona_instance_mode": TASK_BOUND_INSTANCE_MODE,
        "chat_mode_instances": redirect_targets,
        "next_expected": next_expected,
    }
