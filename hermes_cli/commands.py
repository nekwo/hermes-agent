...
COMMAND_REGISTRY: list[CommandDef] = [
    ...
    # Fork command. Query-only and explicitly safe mid-run (the fork's Guard-2
    # chain dispatched it unconditionally), so it declares busy_policy=
    # "dispatch" and relies on its normal handler. gateway/run.py's Guard-2
    # "plain" table must carry a "queue-status" entry for that to resolve.
