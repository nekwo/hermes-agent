...
#
# Windows boundary: a literal backslash also terminates the tag. Windows
# ``MEDIA:`` lines routinely travel through transports that leave escape
# sequences undecoded, so a quoted drive path is frequently followed by a
# literal ``\n`` rather than a real newline (e.g. the Stage C MCP screenshot
# lane emits ``MEDIA:'X:\Hermes Cache\shot.png'\nDone``). Without ``\\`` in the
# closing lookahead the tag fails to match and the screenshot is never
# delivered, so the class keeps it alongside upstream's wider markers.
        ...
        for match in media_pattern.finditer(scan_content):
            ...
            if path:
                ...
                try:
                    ...
                    continue
                # expanduser() does not always raise on an embedded NUL, so
                # reject it explicitly before the path reaches any filesystem
                # or upload call. Skip only this tag; other attachments in the
                # same response still deliver.
                if "\x00" in expanded:
                    continue
