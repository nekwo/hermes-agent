...
def _notification_poller_loop(
    ...
    while not stop_event.is_set() and not session.get("_finalized"):
        ...
        # Visibility (the status.update above) is unconditional; the legacy
        # full-agent notification turn is not. When it is off we consume the
        # event here rather than entering the requeue lifecycle below — there
        # is no turn to schedule, so requeueing would only respin the poller.
