...
def apply_planning_decision(task: Task, decision: AgentDecision, *, actor: str, event_log: EventLog | None = None, task_store=None, incident_store=None, proof_store=None, run_id: str | None = None, normal_worker_flow: bool = False, mission_plan_flow: bool = False) -> Task:
    ...
    if mission_plan_flow and actor == "neko_supervisor" and decision.type == DecisionType.PROPOSE_ACCEPTANCE:
        ...
        if target is not None:
            ...
            if target.owner == "qa":
                ...
                task.state = TaskState.PM_READY_FOR_DEV if task.state in {TaskState.CREATED, TaskState.PM_TRIAGE} else TaskState.DEV_IMPLEMENTING
    ...
    if decision.type == DecisionType.PROPOSE_ACCEPTANCE:
        if actor == "neko_supervisor" and task.state == TaskState.DEV_READY_FOR_QA:
            ...
            log.append(
                Event(
                    ...
                    actor,
                    {
                        ...
                        "next_state": TaskState.DEV_READY_FOR_QA.value,
            ...
            return task
        if (
            task.state in {TaskState.QA_APPROVED, TaskState.PM_PROOF_REVIEW, TaskState.PM_READY_FOR_INTEGRATION}
            and task.proof_ids
            ...
            return task
        if task.state in {TaskState.QA_APPROVED, TaskState.PM_PROOF_REVIEW, TaskState.PM_READY_FOR_INTEGRATION} and task.proof_ids:
        ...
        if task.state in {TaskState.PM_READY_FOR_DEV, TaskState.DEV_AUDIT}:
        ...
        if task.state in {TaskState.PM_READY_FOR_DEV, TaskState.DEV_AUDIT, TaskState.DEV_STAGE_PLANNING, TaskState.DEV_TEST_DESIGN, TaskState.DEV_IMPLEMENTING, TaskState.DEV_READY_FOR_QA, TaskState.QA_NEEDS_FIXES}:
            task.state = TaskState.DEV_IMPLEMENTING
        ...
        if (
            ...
            and task.state in {TaskState.QA_APPROVED, TaskState.PM_PROOF_REVIEW, TaskState.PM_READY_FOR_INTEGRATION}
            and task.proof_ids
            and _all_stages_passed(task)
            and not _needs_cross_stack_launcher_completion(task, proof_store=proof_store)
        ):
            ...
            return task
        if decision.type == DecisionType.APPROVE and task.state in {TaskState.QA_APPROVED, TaskState.PM_PROOF_REVIEW, TaskState.PM_READY_FOR_INTEGRATION} and task.proof_ids:
            _advance_to_next_dev_stage(task)
            task.state = TaskState.DEV_IMPLEMENTING
            task.updated_at = now()
        ...
        if not _all_stages_dev_complete(task):
            _advance_to_next_dev_stage(task)
        task.state = TaskState.DEV_READY_FOR_QA if _all_stages_dev_complete(task) else TaskState.DEV_IMPLEMENTING
        task.updated_at = now()
        ...
        if task.state in {TaskState.DEV_IMPLEMENTING, TaskState.QA_NEEDS_FIXES} or proof_ids:
    ...
    log.append(
        Event(
            ...
            actor,
            {
                ...
                "next_state": TaskState.PM_READY_FOR_DEV.value,
                "next_expected": "launcher_dev_verification",
...
def _dev_replanned_materialized_proof_stage(task: Task, *, actor: str) -> bool:
    ...
    if state not in {TaskState.DEV_IMPLEMENTING, TaskState.QA_NEEDS_FIXES}:
        return False
