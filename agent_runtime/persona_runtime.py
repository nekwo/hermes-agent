...
def _mission_chat_operative_rules() -> str:
    ...
    return (
        ...
        "them with `agent_chat_send` carrying the `session_id` their reply returned, so the exchange continues as one "
        "conversation — don't drop their question, answer it by guessing, or send the answer without that session_id (a send "
        "with no session_id opens a NEW thread and they lose the question's context).\n"
        ...
        "continues your durable pair thread when you genuinely want one running conversation rather than a task. To recall "
