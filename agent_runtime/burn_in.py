...
STAGE47_CASES: dict[str, dict[str, Any]] = {
    "noop-orchestration": {
        ...
        "description": "Prove Neko Mission Lead, Backend Dev, Neko join, Launcher Dev, and QA routing without product edits.",
        "acceptance_criteria": [
            "Backend Dev receives the first no-op route and attaches proof without modifying product files.",
            "Neko joins the backend proof and releases Launcher Dev only after backend proof exists.",
            "Launcher Dev receives the second no-op route and attaches proof without modifying product files.",
            "QA independently reviews the attached proof IDs and produces an evidence-backed QA verdict.",
            "No invalid-output incidents occur.",
        ],
        "affected_repos": ["EterniaBackend", "EterniaLauncher", "hermes-agent"],
        "suggested_roles": ["neko_supervisor", "backend_dev", "neko_supervisor", "dev", "qa"],
        "risk_flags": ["cross_stack_sequential_handoff", "backend_contract_first", "routing_burn_in_only", "no_product_edits", "proof_ids_required_before_qa"],
        "non_goals": ["No product code edits.", "No repository refactors.", "No visual proof required.", "No live product behavior changes."],
        "expected_persona_sequence": ["neko_supervisor", "backend_dev", "neko_supervisor", "dev", "qa"],
    },
    "backend-only-edit": {
        "title": "Stage 47 backend-only burn-in",
        "description": "Prove a bounded backend-only implementation and proof cycle.",
        "acceptance_criteria": ["Backend proof is attached.", "QA reviews backend proof."],
        "expected_persona_sequence": ["neko_supervisor", "backend_dev", "qa"],
    },
    "launcher-only-edit": {
        "title": "Stage 47 Launcher-only burn-in",
        "description": "Prove a bounded Launcher implementation and proof cycle.",
        "acceptance_criteria": ["Launcher proof is attached.", "QA reviews Launcher proof."],
        "expected_persona_sequence": ["neko_supervisor", "dev", "qa"],
    },
    "cross-stack-edit": {
        "title": "Stage 47 cross-stack burn-in",
        "description": "Prove a complex backend-first, Launcher-second multi-agent contract workflow with proof-backed Neko joins and QA review.",
        "acceptance_criteria": [
            ...
            "Neko joins the backend proof before releasing Launcher Dev.",
            "Launcher Dev consumes the joined backend proof and attaches deterministic Launcher proof.",
            "QA reviews both backend and Launcher proof IDs and gives an evidence-backed verdict.",
        ],
        "affected_repos": ["EterniaBackend", "EterniaLauncher", "hermes-agent"],
        "suggested_roles": ["neko_supervisor", "backend_dev", "neko_supervisor", "dev", "qa"],
        "risk_flags": [
        ...
        "non_goals": [
            ...
            "Do not proceed to QA until both backend and Launcher proof IDs are attached.",
        ],
        "expected_persona_sequence": ["neko_supervisor", "backend_dev", "neko_supervisor", "dev", "qa"],
    },
