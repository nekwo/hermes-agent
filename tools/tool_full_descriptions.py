...
FULL_TOOL_DESCRIPTIONS: Dict[str, Union[str, Callable[[], str]]] = {
    ...
    'agent_chat_send': (
        "Send a chat message to ANOTHER Harness persona (agent-to-agent chat). Use this when the operator asks you to brief, prompt, deploy, hand off to, or check in with another agent conversationally — e.g. 'have Neko look at X', 'tell the dev agent to prepare Y'. The message lands in that persona's own Mission Control chat session and their reply is returned to you. This does NOT create a tracked goal, start the Mission Daemon, or run proof gates — use mission_goal_create when the operator wants real tracked work to start. Prefer the persona id (e.g. neko_supervisor, dev, backend_dev, qa), which reaches that persona's canonical primary instance. When a persona runs MORE THAN ONE live instance, pass the specific @personainst_* handle from your Runtime Situation HUD to reach THAT instance exactly (its own thread), not the primary. Display names are not accepted.\n\nThread control — threads are TASK-SCOPED (V3, 2026-07-27):\n- OMIT session_id → under the default `new_per_dispatch` policy this MINTS a fresh thread scoped to this task, named after your message (or after the `title` you pass). Dispatch a new task and just send; no flag.\n- session_id=<id> → continue THAT exact thread. Pass back the session_id the reply returned whenever you are staying in one exchange (answering their clarifying question, an in-task follow-up, a correction) — omitting it opens ANOTHER fresh thread and they read your message with no context attached.\n- new_session=false → continue the target's CURRENT default thread (the most recently established one) instead of opening a task thread. That pointer is NOT a stable per-pair thread: every fresh dispatch repoints it, so `new_session:false` follows whichever thread ran last. To continue a SPECIFIC conversation, pass its session_id.\n- new_session=true → force a fresh thread where the policy would not have opened one. Passing new_session=true together with session_id is contradictory and refused.\nEvery reply carries a `session_established` block {fresh, reason, predecessor_session_id}: which thread you landed in, why, and the thread it superseded. Use agent_chat_threads to list your threads and agent_chat_open to review a thread's recent messages before continuing it. To recall earlier work with a teammate, search past sessions (session_search) rather than keeping an unrelated task thread alive as memory."
    ),
    'clarify': (
