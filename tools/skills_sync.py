...
def _hermes_home() -> Path:
    """The ACTIVE profile home, resolved at call time.

    Same contract as ``_skills_dir()`` one name up: the module-level constant
    stays the override seam (tests and external patchers set it), and an
    unpatched module follows the live profile.
    """
...
def _manifest_file() -> Path:
    """The ACTIVE profile's bundled manifest, resolved at call time.

    ``MANIFEST_FILE`` is a patch seam in its own right (some callers point the
    manifest somewhere other than ``<skills>/.bundled_manifest``), so it wins
    when set; otherwise the manifest follows whichever skills dir is live.
    """
...
def _backfill_optional_provenance(quiet: bool = False) -> List[str]:
    ...
    for skill_md in sorted(optional_dir.rglob("SKILL.md")):
        ...
        if lock_name in installed or install_path in existing_paths:
            continue
        # Live-resolved skills root (profile-aware), not the import-time
        # constant: a HERMES_HOME switch after import must not send provenance
        # repair at the previous profile's tree.
        dest = _skills_dir() / Path(*install_path.split("/"))
...
def _rmtree_writable(path: Path) -> None:
    ...
    # Resolved through ``_skills_dir()`` — the SAME accessor every deletion
    # path here computes its ``dest`` from. That is load-bearing: if this guard
    # read a frozen import-time root while the callers followed a live profile
    # switch (or the reverse), the comparison would be between two different
    # homes and the guard would either refuse every legitimate removal or wave
    ...
    # itself must never be removed: a ``dest`` that collapses to the skills
    # root (e.g. a relative path resolving to ``.``) would wipe every
    # installed skill, and its ``.bak`` sibling lands one level up in the
    # profile home. Require a strict-child relationship so both escape into
    # the skills root and out of it are refused.
