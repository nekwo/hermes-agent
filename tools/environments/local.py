...
def _find_windows_git_bash() -> "str | None":
    ...
    Candidate order (the first candidate that actually *starts* wins):
      ...
      3. Derived from the installed ``git`` executable (``_bash_from_git``) —
         covers scoop/choco/winget installs outside the standard dirs.
      4. Standard Git-for-Windows install locations.
      5. ``shutil.which("bash")`` — ONLY when it is not the System32 WSL stub.

    Existence alone is not enough: each candidate is probed with
    :func:`_bash_starts`, so a stale ``HERMES_GIT_BASH_PATH`` pointing at a
    broken Git-for-Windows install (``Directory \\drivers\\etc does not
    exist``) cannot beat a healthy portable Git.  When no candidate starts we
    raise the Mandatory-ASLR/MSYS remediation for that known failure class,
    otherwise we hand back the first candidate so the caller surfaces the real
    bash error rather than a misleading "not found".
    ...
    if from_git and from_git not in candidates:
    ...
    if (
        found
        and not _is_windows_system_shim(found)
    ...
    # Nothing started.  Hand back the first candidate so the caller surfaces
    # the real bash error (or the ASLR remediation ``_find_bash`` raises)
    # instead of the far less useful "not found" message.  This function stays
    # non-raising: ``hermes_cli.dep_ensure`` uses it as a plain availability
    # predicate (``_find_windows_git_bash() is not None``).
...
def _find_bash() -> str:
    ...
    if found:
        # ``_find_windows_git_bash`` returns an unstartable candidate rather
        # than nothing.  The probe is cached, so re-asking is free — and when
        # the failure is the known Git-for-Windows Mandatory-ASLR/MSYS class,
        # raise the targeted remediation instead of letting every later
        # terminal call fail with an opaque spawn error.
        if not _bash_starts(found):
            probe_details = _bash_probe_failure_details(found)
            if _mandatory_aslr_enabled() is True or _looks_like_msys_spawn_failure(
                probe_details
            ):
                raise RuntimeError(_git_bash_aslr_help(found, probe_details))
        return found
...
def _make_run_env(env: dict) -> dict:
    ...
    if path_key is not None:
        ...
        # …then guarantee the native command tooling (cmd.exe, powershell.exe,
        # pwsh, ssh) is reachable so the agent can shell out to PowerShell/cmd
        # from its bash terminal even under a minimal gateway PATH.  This step
        # only APPENDS, so the Git Bash coreutils prepended just above keep
        # precedence over the same-named System32 tools (find.exe, sort.exe).
