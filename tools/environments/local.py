...
def _shell_arg_safe_path(path: str) -> str:
    ...
    ``source``, redirections, ``cd`` — where the MSYS ``/c/...`` spelling is
    exactly right. Program **arguments** are a different consumer class:
    ``rg``, ``python.exe`` and friends on Windows are frequently NATIVE
    binaries with no MSYS runtime, and they cannot resolve ``/c/...`` at all
    (``IO error ... (os error 3)``).

    The two Windows mechanisms in this module otherwise contradict each other
    ...
    has no counterpart and content search is simply broken.
    ...
    * with argument conversion disabled MSYS cannot re-mangle it into the
    ...
    paths, relative paths, and non-path arguments such as a regex pattern
    (``func_\\w+``) or a ``python -c`` snippet — is returned **verbatim**, so
    this never rewrites backslashes inside argument text that merely happens
    to contain them.
    """
    if not _IS_WINDOWS or not path:
        return path
