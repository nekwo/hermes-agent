...
TERMINAL_TOOL_DESCRIPTION = "Run shell commands on a persistent Linux env (cwd + exported env persist between calls). Foreground returns instantly; background=true (pair notify_on_complete=true) for servers/long tasks; pty=true for interactive CLIs. Disambiguator: for reading/searching/editing files use read_file/search_files/patch/write_file, not cat/grep/sed/echo."
