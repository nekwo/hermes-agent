...
SESSION_SEARCH_SCHEMA = {
    "name": "session_search",
    "description": (
        "Search or scroll the local session DB of past Hermes conversations (FTS5, no LLM). Shapes: query -> discover sessions; session_id + around_message_id -> scroll a window; session_id -> read one; no args -> recent. Disambiguator: searches conversation HISTORY, not a live source -- if the user gave a URL/file/account, inspect that first. Call tool_describe for the full shape/FTS5 reference."
    ),
    "parameters": {
        "type": "object",
        "properties": {
