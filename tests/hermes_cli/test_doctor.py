...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("kimi-coding", "kimi-k2"),
        # ``nvidia`` and ``moa`` come from the shadowed copy of this test that
        # was deleted on 2026-09-04. They were never collected there (the later
        # definition of the same name won), so this is the first run they get.
...
def test_run_doctor_accepts_hermes_provider_ids_that_catalog_aliases(
    ...
    # ``nvidia`` joins the vendor-slug arm because the deleted copy asserted it
    # there; ``moa``'s default carries a vendor prefix that copy did not assert
    # about, so it stays out of this set exactly as it was.
