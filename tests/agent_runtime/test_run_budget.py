...
# ---------------------------------------------------------------------------
# The block reaches the RUN RECORD, not only profile_timing
# ---------------------------------------------------------------------------
#
# ``profile_timing`` is an ``_ms``/``_count`` INTEGER map with exactly one
# structured entry, and ``persona_runtime._record_timing_value`` filters to
# scalars — so the chat/worker run record silently lost the only answer to
# "what bounded this run?" while the CLI envelope (which copies profile_timing
# wholesale) kept it. Wave-4 deferred this as a one-line class of fix; these
# pin the fix and the two contracts it must not break.


def _persona_run() -> "AgentRun":
    ...
    ts = now()
    return AgentRun(
        id="run_budget_record",
        persona_id="qa",
        task_id="task_budget_record",
        stage_id=None,
...
def test_the_run_record_carries_the_whole_accounting_block():
    ...
    block = run.llm["run_budget"]
    assert block == result.profile_timing["run_budget"]
    # The WHOLE block, not a scalar summary of it.
    assert set(block) >= {"bounded_by", "trip_reason", "enforcement", "tripped", "budgets"}
    assert isinstance(block["budgets"], list)


def test_the_timing_map_keeps_its_integer_contract():
    """The block rides as its own key, never smuggled into ``timing``: every
    consumer of ``run.llm["timing"]`` may keep assuming scalars."""

    from agent_runtime.persona_runtime import _apply_llm_metadata

    run = _persona_run()

    _apply_llm_metadata(run, _run(_Agent))

    timing = run.llm["timing"]
    assert timing
    assert all(isinstance(value, int) for value in timing.values())
    assert "profile_run_budget" not in timing
    assert "run_budget" not in timing


def test_a_later_result_without_a_block_does_not_erase_the_recorded_one():
    """Same carry-forward the timing map already gets."""
    ...
    run = _persona_run()
    _apply_llm_metadata(run, _run(_Agent))
    recorded = run.llm["run_budget"]

    _apply_llm_metadata(
        run,
        AgentRunResult(
            final_response="ok",
            session_id="session_run_budget",
            provider=None,
            model=None,
            base_url=None,
            messages=[],
    ...
    assert run.llm["run_budget"] == recorded


def test_a_run_that_never_declared_a_budget_records_no_claim():
    """Absent is absent: no empty block on the record."""
    ...
    assert "run_budget" not in run.llm
