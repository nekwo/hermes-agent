...
def test_active_runs_include_queued_starting_running_and_waiting_states():
    ts = now()
    task = Task(id="task_active", title="Mission", description="d", state=TaskState.READY_FOR_IMPLEMENTATION, created_at=ts, updated_at=ts, requested_by="human")
    runs = [
...
def test_active_run_summary_includes_current_tool_command_label():
    ts = now()
    task = Task(id="task_active_tool", title="Mission", description="d", state=TaskState.READY_FOR_IMPLEMENTATION, created_at=ts, updated_at=ts, requested_by="human")
    run = AgentRun(
        id="run_active_tool",
        persona_id="dev",
        task_id=task.id,
