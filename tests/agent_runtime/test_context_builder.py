...
def test_dev_recovery_context_carries_latest_cross_stage_qa_review():
    log = EventLog()
    task = make_task()
    task.state = TaskState.QA_NEEDS_FIXES
    task.current_stage_id = "backend_investigation"
    task.stages = [
...
def test_normal_worker_flow_committed_verification_stage_prefers_request_gate():
    task = make_task()
    task.title = "Improve Stage 53 goal runner final receipts rerun"
    task.description = (
    ...
    task.stages[0] = TaskStage(
        ...
        objective="Verify the committed hermes-agent Stage 53 goal runner receipt fix and produce focused command proof.",
        status=StageStatus.IMPLEMENTING,
        affected_paths=[
        ...
        test_plan=["python -m pytest tests/agent_runtime/test_goal_runner.py tests/agent_runtime/test_planning.py -q"],
    )
    ...
    assert hud["primary_worker_action"]["action_id"] == "request_gate"
    assert hud["next_required_move"]["shape_id"] == "dev.request_test_run"
    assert hud["decision_menu"][0]["shape_id"] == "dev.request_test_run"
    ...
    assert "dev.propose_patch" not in [item["shape_id"] for item in hud["decision_menu"]]


def test_normal_worker_flow_exposes_request_gate_for_no_edit_command_stage():
    task = make_task()
    ...
    task.stages[0] = TaskStage(
        ...
        status=StageStatus.IMPLEMENTING,
        acceptance_criteria=["The verifier command passes."],
        test_plan=["python -m hermes_cli.main harness contracts verify-examples --json"],
    )
    task.current_stage_id = "stage49_contract_registry_verify_examples"
    run = make_run()
    run.stage_id = "stage49_contract_registry_verify_examples"
    cfg = RuntimeConfig(normal_worker_flow=NormalWorkerFlowConfig(enabled=True))
...
def test_normal_worker_flow_exposes_recipe_gate_for_no_edit_recipe_stage():
    task = make_task()
    ...
    task.stages[0] = TaskStage(
        ...
        objective="Collect redaction-safe Harness status and snapshot proof.",
        status=StageStatus.IMPLEMENTING,
        acceptance_criteria=["The existing Harness status snapshot recipe passes."],
        test_plan=[],
    )
    task.current_stage_id = "harness_runtime_status_snapshot"
    run = make_run()
    run.stage_id = "harness_runtime_status_snapshot"
    cfg = RuntimeConfig(normal_worker_flow=NormalWorkerFlowConfig(enabled=True))
...
def test_normal_worker_flow_infers_recipe_gate_from_generic_no_edit_stage():
    task = make_task()
    ...
    task.stages[0] = TaskStage(
        ...
        status=StageStatus.IMPLEMENTING,
        acceptance_criteria=[
            "No backend product files are inspected or changed.",
            "Backend proof ID is joined before Launcher Dev starts UI/bridge repair.",
        ],
        test_plan=["Run/request the existing backend_contract_smoke no-product-edit proof recipe."],
        audit_notes=[
            "backend_dev: Current HUD required_next_decision=propose_patch conflicts with the no-edit backend_contract_smoke proof stage.",
        ],
        corrections=[
            "backend_dev: Do not require changed_files, patch content, or product-edit delivery for this stage.",
        ],
    )
    task.current_stage_id = "eterniabackend_fresh_scope"
    run = make_run()
    run.persona_id = "backend_dev"
    run.stage_id = "eterniabackend_fresh_scope"
    cfg = RuntimeConfig(normal_worker_flow=NormalWorkerFlowConfig(enabled=True))
    ...
    assert hud["decision_contract_mode"] == "normal_worker_flow"
    assert hud["primary_worker_action"]["action_id"] == "request_gate"
    assert hud["next_required_move"]["recommended_payload"] == {
        "stage_id": "eterniabackend_fresh_scope",
        "recipe_id": "backend_contract_smoke",
    }
    assert hud["decision_menu"][0]["shape_id"] == "dev.request_test_run"
    assert hud["decision_menu"][0]["recommended_payload"]["recipe_id"] == "backend_contract_smoke"
    assert "dev.propose_patch" not in [item["shape_id"] for item in hud["decision_menu"]]


def test_typed_plan_backend_proof_stage_hud_exposes_request_gate_not_patch():
    task = make_task()
    task.mission_plan = MissionPlan(
        ...
        stages=[
            MissionPlanStage(
                ...
                objective="Prove backend stream seed before Launcher UI repair.",
                owner="backend_dev",
                repo="EterniaBackend",
                kind="proof_only",
            ...
            MissionPlanStage(
                ...
                title="Launcher UI Repair",
                objective="Repair Launcher UI.",
                owner="dev",
    ...
    task.current_stage_id = "backend_contract_smoke"
    run = make_run()
    run.persona_id = "backend_dev"
    run.stage_id = "backend_contract_smoke"
    cfg = RuntimeConfig(
    ...
    assert hud["typed_current_stage"]["id"] == "backend_contract_smoke"
    assert hud["primary_worker_action"]["action_id"] == "request_gate"
    assert hud["decision_menu"][0]["shape_id"] == "dev.request_test_run"
    assert hud["decision_menu"][0]["recommended_payload"]["recipe_id"] == "backend_contract_smoke"
    assert "dev.propose_patch" not in [item["shape_id"] for item in hud["decision_menu"]]
...
def test_typed_plan_backend_context_diagnostic_without_gate_requests_context():
    task = make_task()
    task.affected_repos = ["EterniaBackend"]
    task.mission_plan = MissionPlan(
        mission_intent=MissionIntent(
            title="Backend Dev efficiency smoke",
    ...
    task.current_stage_id = "diagnostic_backend_dev"
    run = make_run()
    run.persona_id = "backend_dev"
    run.stage_id = "diagnostic_backend_dev"
    cfg = RuntimeConfig(
        normal_worker_flow=NormalWorkerFlowConfig(enabled=True),
        mission_plan=MissionPlanConfig(enabled=True),
    )
    ...
    assert hud["typed_current_stage"]["owner"] == "backend_dev"
    assert hud["primary_worker_action"]["action_id"] == "request_context"
    assert hud["next_required_move"]["shape_id"] == "common.request_file_reads"
    assert hud["agent_hud"]["recommended_action"]["shape_id"] == "common.request_file_reads"
    assert "dev.request_test_run" not in [item["shape_id"] for item in hud["decision_menu"]]
    assert "common.block" in [item["shape_id"] for item in hud["decision_menu"]]


def test_typed_plan_backend_investigation_context_stage_hud_requests_context_not_gate():
    task = make_task()
    task.affected_repos = ["EterniaBackend"]
    task.mission_plan = MissionPlan(
        mission_intent=MissionIntent(
            title="NSFW filter leakage investigation",
            objective="Investigate backend moderation paths and produce a staged hardening plan.",
        ),
        ...
        stages=[
            MissionPlanStage(
                ...
                objective="No-product-edit investigation: inspect backend NSFW moderation paths, thresholds, queues, and fallback behavior.",
                owner="backend_dev",
                repo="EterniaBackend",
                kind="context",
                requires_product_edit=False,
...
def test_typed_plan_backend_investigation_hud_delivers_after_repeated_context():
    task = make_task()
    task.affected_repos = ["EterniaBackend"]
    task.mission_plan = MissionPlan(
        mission_intent=MissionIntent(
    ...
    task.context_requests = [
        {
            ...
            "stage_id": "backend_investigation",
            "status": "fulfilled",
            "reason": "backend_investigation first bundle",
        },
        {
            "id": "ctx_2",
            "actor": "backend_dev",
            ...
            "reason": "backend_investigation second bundle",
        },
    ]
    run = make_run()
    run.persona_id = "backend_dev"
...
def test_typed_plan_launcher_context_diagnostic_without_gate_requests_context():
    task = make_task()
    task.affected_repos = ["EterniaLauncher"]
    task.mission_plan = MissionPlan(
        mission_intent=MissionIntent(
            title="Launcher Dev efficiency smoke",
            objective="No-edit Launcher Dev efficiency smoke.",
        ),
        current_stage_id="diagnostic_dev",
        stages=[
            MissionPlanStage(
                id="diagnostic_dev",
                title="dev Diagnostic",
                objective="No-edit Launcher Dev efficiency smoke. Do not inspect code or edit files.",
                owner="dev",
                repo="EterniaLauncher",
                kind="context",
                requires_product_edit=False,
    ...
    task.current_stage_id = "diagnostic_dev"
    run = make_run()
    run.stage_id = "diagnostic_dev"
    cfg = RuntimeConfig(
        normal_worker_flow=NormalWorkerFlowConfig(enabled=True),
        mission_plan=MissionPlanConfig(enabled=True),
    )
    ...
    assert hud["typed_current_stage"]["owner"] == "dev"
    assert hud["primary_worker_action"]["action_id"] == "request_context"
    assert hud["next_required_move"]["shape_id"] == "common.request_file_reads"
    assert hud["agent_hud"]["recommended_action"]["shape_id"] == "common.request_file_reads"
    assert "dev.request_test_run" not in [item["shape_id"] for item in hud["decision_menu"]]
...
def test_typed_plan_harness_status_snapshot_stage_hud_exposes_request_gate_not_patch():
    task = make_task()
    task.affected_repos = ["hermes-agent"]
    task.mission_plan = MissionPlan(
        mission_intent=MissionIntent(
            title="Harness thinking log smoke",
            objective="Verify redaction-safe Harness thinking process logs.",
        ),
    ...
    assert hud["typed_current_stage"]["id"] == "harness_runtime_status_snapshot"
    assert hud["primary_worker_action"]["action_id"] == "request_gate"
    assert hud["decision_menu"][0]["shape_id"] == "dev.request_test_run"
    assert hud["decision_menu"][0]["recommended_payload"] == {
        "stage_id": "harness_runtime_status_snapshot",
...
def test_typed_plan_stale_harness_log_implementation_stage_self_heals_to_request_gate():
    task = make_task()
    task.affected_repos = ["hermes-agent"]
    stale_stage_id = "verify_the_hermes_harness_mission_control_log_snapshot_contract"
    task.mission_plan = MissionPlan(
        mission_intent=MissionIntent(
            title="Harness thinking log smoke",
            objective="Verify redaction-safe Harness thinking process logs without product edits.",
        ),
        current_stage_id=stale_stage_id,
        stages=[
            MissionPlanStage(
                id=stale_stage_id,
                title="Implementation",
    ...
    task.current_stage_id = stale_stage_id
    run = make_run()
    run.stage_id = stale_stage_id
    cfg = RuntimeConfig(
        normal_worker_flow=NormalWorkerFlowConfig(enabled=True),
        mission_plan=MissionPlanConfig(enabled=True),
    )
...
def test_dev_mission_hud_requires_stage_plan_when_no_executable_stage_exists():
    task = make_task()
    task.state = TaskState.PM_READY_FOR_DEV
    task.stages = []
