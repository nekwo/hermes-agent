...
* ``observability.build_observability(proofs: list[Proof])`` is LIVE, but both
  ...
  annotation named a record no store can produce, so it is retyped honestly
  rather than deleted; the parameter and its ``proofs_total`` row stay because
...
refs the continuity lane renders into the parent chat, the ``proofs_total``
observability row, and ``parity`` / ``patch_coverage`` (doc 16's named keeps)
are all unaffected.
"""
...
def test_the_observability_proofs_parameter_and_row_are_unchanged():
    """Retyping an annotation must not change the wire. Both live callers pass
    ``[]``, so this row has read ``0`` since S8 and still does."""

    parameters = inspect.signature(observability.build_observability).parameters
    assert "proofs" in parameters

    envelope = observability.build_observability(
        tasks=[], runs=[], incidents=[], proofs=[], daemon_status=None
    )
    assert envelope["signals"]["proofs_total"] == 0
