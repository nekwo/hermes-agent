...
def _bounded_prompt_cache_key(value: Any) -> Optional[str]:
    ...
    Also the single bounding helper for the Codex cache-scope routing headers
    (``session_id`` / ``x-client-request-id``): Codex treats them as its
    prompt-cache scope and rejects values longer than 64 characters. Ids that
    already satisfy the limit are preserved verbatim so existing cache buckets
    stay stable; only oversized ids are content-addressed, which lets durable
    runtime identifiers (persona-chat scope ids) grow without breaking calls.
    """
    if value is None:
        return None
    key = str(value).strip()
    ...
    def build_kwargs(
        ...
        **params,
        ...
        if is_codex_backend:
            ...
            # Bounded through the same helper as prompt_cache_key so the 64-char
            # provider limit is enforced from a single place. The ``cache_scope_id``
            # param itself is left untouched — the observability block below
            # reports the *source* of the scope, not the bounded header value.
            scope_header_value = _bounded_prompt_cache_key(
        ...
        # Persist only one-way fingerprints of the FINAL routing values. Runs
        # after every bounding/override step above so it describes what the SDK
        # will actually send. The profile runner copies this into redaction-safe
        # prompt observability so a cold turn can be compared with the preceding
        # warm turn without exposing session ids or request headers.
        # Compare against the BOUNDED key: upstream's provider-limit bounding
        # runs between key computation and this capture, so an oversized
        # session-fallback key would otherwise be misreported as an operator
        # request_override. Short keys (the content-addressed pck_ form) are
        # preserved verbatim, so "static_prefix" is unaffected.
        self._last_cache_routing_observability = _cache_routing_observability(
            kwargs,
            computed_cache_key=_bounded_prompt_cache_key(cache_key) or cache_key,
