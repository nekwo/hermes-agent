        ...
        if agent._session_db:
            ...
            try:
                ...
                if in_place:
                    ...
                    # Persona-chat continuity: a rotation child of a Mission
                    # Control chat must keep pointing at the SAME chat root and
                    # persona instance, otherwise the runtime's read model loses
                    # the conversation at the compression boundary. Inherit those
                    # keys from the root chat's model_config (the live agent's
                    # own _session_init_model_config may predate them). Absent on
                    # every non-persona lane, where this is a no-op.
                    ...
                    try:
                        ...
                        pass
                    # No separate create_session/orphan-rollback here any more:
                    # publish_compression_child above closes the parent and
                    # publishes the child in ONE transaction, so the orphaned-child
                    # failure mode the fork rolled back from (#33906/#33907) can no
                    # longer be observed — a failed publish raises with the parent
                    # still live and the id unrotated.
                    agent._session_db_created = True
