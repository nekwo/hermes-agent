...
def build_turn_context(
    ...
    *,
    ...
    # appending a second native user message. That lane deliberately leaves
    # ``_persist_user_message_idx`` unset (the row is already durable, so the
    # crash persist must not write a duplicate) and never stamps display
    # metadata — the staged ``user_msg`` is discarded there, and it may be the
    # agent's still-referenced ``_pending_cli_user_message`` dict.
