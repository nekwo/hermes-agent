...
def run_codex_stream(agent, api_kwargs: dict, client: Any = None, on_first_delta=None):
    ...
    for attempt in range(max_stream_retries + 1):
        ...
        try:
            # Fork metering wraps the relay-managed open: the timing seam is
            # "how long did it take to get a stream handle", which is now
            # relay_llm.stream(...) rather than a bare responses.create().
        ...
        # Compatibility with mocks/providers that return a concrete response
        # instead of an iterable is now handled by relay_llm's
        # ``completed_response_predicate`` (it parks the object on
        # ``final_response`` and hands back an empty iterator); the
        # ``except RuntimeError`` branch below returns it.
        ...
        try:
            try:
                ...
                if event_stream.final_response is not None:
                    # The provider handed back a complete response with no SSE
                    # frames (mock/compat path). Keep the fork's metering row so
                    # a zero-event turn is still accounted for rather than
                    # silently missing from provider timings.
                    _emit_provider_timing(
                        agent,
                        "concrete_response",
                        0,
