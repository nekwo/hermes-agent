...
# v2 was minted independently on both sides of the upstream sync and means two
# different payload shapes: upstream v2 entries gained org provenance fields
# (org_id/org_author/rel_dir) for M2 org-shared skills, fork v2 entries gained
# the ``runtime`` block (metadata.hermes surfaces/modes/load_policy). The merged
# entry is the union, so neither historical v2 snapshot can be trusted — v3
# forces one cold rebuild and every entry then carries both.
...
def build_context_files_prompt(
    ...
    if (
        ...
        # Deterministic project context: load EVERY supported source that
        # exists, in the documented order. Upstream stops at the first match;
        # the fork treats .hermes.md / AGENTS.md / CLAUDE.md / .cursorrules as
        # complementary, so a repo carrying more than one does not silently
        # lose the lower-priority ones.
