...
| S11 | complete | Persona/profile data is authoritative: bundled seed/catalog constants, role tool ceilings, decision-role filtering, MCP role floor, and the legacy task-bound chat-root split are removed. Red proof: the new declaration/unknown-role tests failed `2`; green: focused S11 slice `162 passed`, exact MCP gate `72 passed`, and the removal grep is empty outside this plan. Live Alice roster remains `11` chat instances (`15` total including `4` configured rows). The live shared runtime-model skill and its repo copy now describe the single chat lane. |
| S12 | pending | Must remain its own revertable security-posture commit. |
