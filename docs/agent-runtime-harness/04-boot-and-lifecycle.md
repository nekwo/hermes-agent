...
`_cmd_serve` starts a `BootTimeline` as its first instruction (`serve.py:3251-3257`).
...
- `head_publish_ms` — `publish_chat_head_home()`, the ONE writer of the shared chat-head pointer
  (`serve.py:1183-1186`). Without it a later plain CLI turn degrades to its own profile database
...
- `root_anchor_ms` — publishes `agent_runtime.store_root` into the platform-default home's
  ...
  root instead of a `%LOCALAPPDATA%` shadow (`serve.py:1187-1218`). Injected and OFF unless the
...
**Orphaned turns** (`orphaned_turn_sweep_ms`, `serve.py:1639-1646` →
...
**Detached dispatches** (`dispatch_restore_ms`, `serve.py:1653-1662`). Same moment, same reason:
...
2. `consult` returns immediately unless `lane_armed()` (`core_cache.py:3112-3133`). The riders
...
4. Miss → `_log_demote` with a reason from the `DEMOTE_*` vocabulary (`core_cache.py:291-305`).
...
5. A cache hit ALSO starts `maybe_start_shadow_validation` (`snapshot.py:601`): the full build
...
6. On a full build: `pre_build_fingerprint()` (the consult's own key, reused — an OLDER key can
   ...
   (`snapshot.py:703`), which disarms the lane (`:659-703`).
...
**labeled stale** while the build runs (`core_cache.py:3165`, `stream.py:948-952`). The one-shot
