...
`_cmd_serve` starts a `BootTimeline` as its first instruction (`serve.py:4868-4870`).
...
- `chat_registry_ms` — the persona-chat hot-session registry, sized from
  `load_root_runtime_config().persona_chat` (`serve.py:1959-1965`).
- `head_publish_ms` — `publish_chat_head_home()`, the ONE writer of the shared chat-head pointer
  (`serve.py:1973-1976`). Without it a later plain CLI turn degrades to its own profile database
...
**Orphaned turns** (`orphaned_turn_sweep_ms`, `serve.py:2704` →
...
**Detached dispatches** (`dispatch_restore_ms`, `serve.py:2720`). Same moment, same reason:
...
ops manifest, and `boot_timeline` (`serve.py:1730-1774`). The blocks are **always present, never
...
actor prewarm (Stage 9a) third — all on the same thread (`serve.py:1611-1621`, `:1060-1086`).
...
(`serve.py:1551`). Naming the caller is what makes this build appear in the log at all:
...
`serve.py::_room_wants_stale_first` (`:3118`) — because a boot starts two `stream_frames`
