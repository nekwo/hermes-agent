...
(`GRANTABLE_COMMAND_CLASSES`), mirroring
`mcp_admission.R1_ADMISSIBLE_ROLES`. Naming one under `grants` is a typed
config error, not a grant. Widening the floor is a product decision, not a
config edit.
